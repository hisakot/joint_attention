...
def train(train_dataloader, swin_unet, loss_function, optimizer, device):
    '''
    resnet50.train()
    unet.train()
    fuse.train()
...
def evaluate(val_dataloader, swin_unet, loss_function, device):
    '''
    resnet50.eval()
    unet.eval()
    fuse.eval()
        ...
        try:
            ...
            if val_loss < early_stopping[0]:
                ...
                torch.save({"epoch" : epoch + 1,
                            ...
                            }, "save_models/img_gazecone_swin_unett_best.pth")
            else:
                early_stopping[2] += 1
