...
class TransGAN_LSTM(nn.Module):
    def __init__(self, patch_size, emb_size, num_heads, forward_expansion, img_height, img_width, in_ch, seq_len):
        super(TransGAN_LSTM, self).__init__()
        ...
        # LSTM block
        self.lstm = LSTMBlock(emb_size*200, emb_size, seq_len)
    ...
    def forward(self, x):
        ...
        for seq in range(seq_len):
            ...
            inp = F.interpolate(inp, self.input_size, mode='bilinear', align_corners=False)

            # Patch Embedding
            inp = self.embedding(inp)  # (B, L, emb_size)

            # Transformer
            inp = self.encoder1(inp)
            inp = self.encoder2(inp)
            inp = self.encoder3(inp) # (B, 200, 512)
            inp = inp.view(B, 1, -1) # (B, seq, input_dim)
            lstm_input.append(inp)

        # LSTM
        lstm_input = torch.cat(lstm_input, dim=1) # (B, 3, 102400)
        x = self.lstm(lstm_input) # (B, 200, 512)
