...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.3): # FIXME default drop=0.
        super().__init__()
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_height=1920, img_width=3840, patch_size=4, in_chans=3, embed_dim=96, norm_layer=None):
        super().__init__()
        img_size = (img_height, img_width)
        patch_size = to_2tuple(patch_size)
        patches_resolution = [img_height // patch_size[0], img_width // patch_size[1]]
...
class SwinTransformerSys(nn.Module):
    ...
    def __init__(self, img_height=1920, img_width=3840, patch_size=4, in_chans=3, num_classes=1000,
        ...
        patches_resolution_x = self.patch_embed_x.patches_resolution
        self.patches_resolution_x = patches_resolution_x

        self.patch_embed_y = PatchEmbed(
                    img_height=img_height, img_width=img_width, patch_size=patch_size, \
                    in_chans=2, embed_dim=embed_dim,
                    norm_layer=norm_layer if self.patch_norm else None)
        num_patches = self.patch_embed_y.num_patches
        ...
        self.pos_drop_x = nn.Dropout(p=drop_rate)
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(dim=int(embed_dim * 2 ** i_layer),
                               input_resolution=(patches_resolution_x[0] // (2 ** i_layer),
        ...
        for i_layer in range(self.num_layers):
            ...
            if i_layer == 0:
                layer_up = PatchExpand(
                    input_resolution=(patches_resolution_x[0] // (2 ** (self.num_layers - 1 - i_layer)),
                                      patches_resolution_x[1] // (2 ** (self.num_layers - 1 - i_layer))),
                    dim=int(embed_dim * 2 ** (self.num_layers - 1 - i_layer)), dim_scale=2, norm_layer=norm_layer)
            else:
                layer_up = BasicLayer_up(dim=int(embed_dim * 2 ** (self.num_layers - 1 - i_layer)),
                                         input_resolution=(
                                         patches_resolution_x[0] // (2 ** (self.num_layers - 1 - i_layer)),
                                         patches_resolution_x[1] // (2 ** (self.num_layers - 1 - i_layer))),
    ...
    def forward_up_features(self, x, x_downsample):
        for inx, layer_up in enumerate(self.layers_up):
            x = layer_up(x) # TODO no skip connection ver
            # if inx == 0: # TODO original condition
            '''
            if inx == 0 or inx > 1:
                x = layer_up(x)
            else:
    ...
    def forward(self, inp):
        if inp.size()[2] == 1: # if C == 1
            inp = inp.repeat(1,1,3,1,1) # C = 3
        ...
        for seq in range(seq_len):
            x_inp = x[:, seq, :, :, :] 
            x_inp = x_inp.view(B, 3, H, W)
            x_inp, x_inp_downsample = self.forward_features_x(x_inp) # (B, 200, 768)
            x_list.append(x_inp)
            x_downsample_list.append(x_inp_downsample)

            y_inp = y[:, seq, :, :, :] 
            y_inp = y_inp.view(B, 2, H, W)
            y_inp, y_inp_downsample = self.forward_features_y(y_inp) # (B, 200, 768)
            y_list.append(y_inp)
            y_downsample_list.append(y_inp_downsample)

        inp_x = torch.stack(x_list, dim=1) # (B, seq_len, 200, 768)
        inp_x = inp_x.permute(0, 2, 1, 3) # (B, 200, seq_len, 768)
